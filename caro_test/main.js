...
function caculateWinner(rowNum, colNum, letter) {
	...
	let point_1 = 1; //rule là point = 5 thi win
	
	let tempRowNum = rowNum;
	let tempColNum = colNum;
	let tempRowNum_dig = rowNum;
	let tempColNum_dig = colNum;
    ...
    while(tempColNum_dig > -1 && tempColNum_dig < 20 && tempRowNum_dig > -1 && tempRowNum_dig < 20) {
		
		curBoxLetter = boxArray[tempRowNum_dig][tempColNum_dig];
		if(curBoxLetter == letter) {
			point++;
		} else {
			//day la chieu di xuong: thoat ra ko kiem tra nua
            ...
            tempRowNum_dig = rowNum;
			tempColNum_dig = colNum;
		}
